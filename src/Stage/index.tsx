...
function Stage({ agentType, onStateChange, onExit, onAuthFail, limitSecond, autoCall = false }: StageProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      controller?.handup();
      useCallStore.getState().reset(true);
    };
      ...
      Dialog.show({
        content: (
          <div style={{ textAlign: 'center' }}>
            由于浏览器安全限制，您需要使用HTTPS访问页面。
            <br />
            如果尚未使用，请切换后再继续。
            <br />
            <a href={`${location.href.replace('http://', 'https://')}`}>使用HTTPS访问</a>
          </div>
        ),
        actions: [],
      });
  ...
  const hasVideo =
    ...
    (agentType === AICallAgentType.AvatarAgent || (agentType === AICallAgentType.VisionAgent));
  ...
  return (
    <div className='stage'>
      ...
      <div className={`stage-bd  ${hasVideo ? 'has-video' : ''}`} onClick={interruptSpeaking}>
        ...
        <Footer onStop={stopCall} onCall={startCall} onOver={stopCall}/>
      </div>
      {callState === AICallState.Error && (
        <Dialog
          visible
