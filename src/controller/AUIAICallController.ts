...
export default abstract class AUIAICallController extends EventEmitter<AUIAICallControllerEvents> {
  ...
  destroy() {
    ...
    this.removeAllListeners();
    if(this._currentScreenShareStream) {
      this.stopScreenShare();
    }
  }

  protected _currentScreenShareStream?: MediaStream;
  public get currentScreenShareStream() {
    return this._currentScreenShareStream;
  }

  /**
   * 切换到屏幕共享
   */
  switchToScreenShare(stream: MediaStream) {
    logger.info('Controller', 'SwitchToScreenShare');
    this._currentScreenShareStream = stream;
    const previewView = this._config.previewView as HTMLVideoElement;
    if(previewView) {
   ...
   */
  stopScreenShare() {
    logger.info('Controller', 'StopScreenShare');
    if (this._currentScreenShareStream) {
      this._currentScreenShareStream.getTracks().forEach(track => track.stop());
      this._currentScreenShareStream = undefined;
    }
    const previewView = this._config.previewView as HTMLVideoElement;
    if(previewView && previewView.srcObject !== this._currentScreenShareStream) {
      previewView.srcObject = null;
    }
  }
}
